...
## 2026-04-15 - Replace O(N*M) nested array lookups with O(1) hash maps
**Learning:** Rewriting a nested array `.find()` into a manual `for` loop does not solve the algorithmic bottleneck of O(N*M) complexity, it merely avoids the callback allocation. The true performance win is reducing algorithmic complexity.
**Action:** When facing O(N*M) nested lookups over static or slowly changing reference arrays, always pre-compute a lookup hash map (using `Object.create(null)` to avoid prototype collisions) so the nested lookup becomes O(1).

...
## 2025-05-23 - Use O(1) Map Lookups Instead of Array.prototype.find
**Learning:** In high-frequency rendering or computational functions (like `getPadraoApplyContext`), iterating over an array using `Array.prototype.find` (e.g., `ATIV_DOMAINS.find(x => x.id === id)`) inside another loop (e.g., `.filter` or `for` loop) creates O(N*M) time complexity. This is significantly slower than using an O(1) hash map lookup, especially as array sizes grow or the function is called frequently.
**Action:** Replaced O(N) `Array.prototype.find()` nested within array iterations with an O(1) hash map lookup (`ATIV_DOMAINS_MAP`) initialized with `Object.create(null)` at the module level.

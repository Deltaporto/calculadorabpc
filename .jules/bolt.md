...
## 2023-11-20 - Avoid Array Spreads and Maps for DOM Collection Initialization
**Learning:** During UI initialization, spreading a NodeList into an array and using `.map()` to instantiate a global `Map` (e.g., `new Map([...document.querySelectorAll('.tab')].map(t => [t.dataset.a, t]))`) creates multiple intermediate array allocations and incurs callback dispatch overhead. While this overhead may seem small, eliminating it across the application improves overall memory pressure.
**Action:** Replace `querySelectorAll` and array spread with `getElementsByClassName` returning a live `HTMLCollection`. Iterate over it using a native `for` loop to manually `.set()` values into the `Map`, avoiding unnecessary memory allocations and leveraging a faster DOM query.

...
## 2025-05-23 - Multi-array Population Loops & O(1) Lookups
**Learning:** Chaining functional array methods like `.filter()` multiple times over the same initial set (e.g., to create multiple distinct arrays like `eligibleEntries`, `manuallyFilledEligible`, and `entriesPreserve`) creates multiple intermediate array allocations and incurs repeated full-iteration costs. Additionally, using `Array.prototype.find()` inside such loops effectively creates an O(N*M) time complexity trap.
**Action:** Replace multiple `.filter()` passes with a single-pass native `for` loop that populates all target arrays simultaneously. In addition, replace O(N) array `.find()` calls on fixed sets with an O(1) lookups via a module-level pre-computed hash map (initialized safely with `Object.create(null)` to avoid prototype collisions).

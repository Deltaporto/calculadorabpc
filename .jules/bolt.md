...
## 2025-05-23 - Avoid Array.prototype.forEach on NodeLists in Hot Paths
**Learning:** During UI initialization and keyboard event handlers (like roving tabindex updates inside `keydown` events), using `.forEach()` on the `NodeList` returned by `document.querySelectorAll()` or `element.querySelectorAll()` creates unnecessary callback dispatch overhead. This is especially problematic in high-frequency event listeners where allocations should be strictly minimized.
**Action:** Replace `querySelectorAll(...).forEach(...)` with native `for (let i = 0; i < elements.length; i++)` loops. This avoids creating an anonymous function on each call and removes the per-element callback dispatch overhead, significantly improving execution speed across thousands of operations.

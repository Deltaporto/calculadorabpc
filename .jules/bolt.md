...
## 2023-11-09 - Avoid Intermediate Array Allocation in Map Initialization
**Learning:** When creating a `Map` from a DOM collection (like `ambTabs`), using array spread syntax `[...document.querySelectorAll('.amb-tab')]` combined with `.map(tab => [+tab.dataset.a, tab])` inside `new Map(...)` allocates two intermediate arrays that are immediately discarded. This creates unnecessary GC pressure during component initialization.
**Action:** Replace `querySelectorAll` and array spreads with a live `HTMLCollection` via `getElementsByClassName()` and use a standard `for` loop to manually populate the `Map` using `Map.prototype.set()`. This entirely avoids the intermediate array allocations.

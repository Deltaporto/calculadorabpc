...
## 2025-10-28 - Replace Multiple Array Filter Passes With Single Loop and Hash Map
**Learning:** Performing multiple `Array.prototype.filter()` passes over the same dataset to populate multiple arrays, and using `Array.prototype.find()` inside the iteration to lookup metadata (e.g. `ATIV_DOMAINS.find`) incurs significant performance penalties due to repeated linear traversals, array allocation overhead, and callback dispatch overhead. This scales poorly in hot paths.
**Action:** Replaced multiple `.filter()` calls and nested `.find()` operations with a single native `for` loop that simultaneously populates all arrays. Extracted the lookup array into a pre-computed module-level hash map (`ATIV_DOMAINS_MAP` using `Object.create(null)`) for O(1) lookups, substantially improving execution speed and reducing garbage collection pressure.

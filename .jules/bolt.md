...
## 2025-10-27 - [Avoid Array.prototype.filter in rendering hot paths]
**Learning:** Using `Array.prototype.filter()` inside state resolution functions (like `resolveCorpoJudFlow`) that run repeatedly during the UI rendering cycle creates intermediate array allocations on every call, increasing GC pressure unnecessarily.
**Action:** Replace chaining of `.filter()` and subsequent `.map()` or iterations with a single native `for` loop that computes the state inline to drastically reduce memory allocation overhead.

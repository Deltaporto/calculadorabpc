...
## 2026-05-18 - [Multi-Array Population Loop in Padrão Médio Context]
**Learning:** In `src/js/main.js`, `getPadraoApplyContext` constructs multiple state arrays (`eligibleEntries`, `manuallyFilledEligible`, `entriesPreserve`) using consecutive `.filter()` passes over the same dataset. Additionally, it performs an O(N) `Array.prototype.find()` over `ATIV_DOMAINS` for each entry to check child age constraints. This "chained filter" pattern incurs intermediate array allocations and O(N*M) nested loop traversal overhead.
**Action:** Replaced the chained `.filter()` calls and `.find()` with a single-pass native `for` loop that populates all target arrays simultaneously. Replaced the nested `.find()` with an O(1) module-level pre-computed hash map (`ATIV_DOMAINS_MAP` initialized via `Object.create(null)`). This completely eliminates intermediate allocations and reduces algorithmic complexity.

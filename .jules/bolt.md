...
## 2026-05-18 - Map initialization Array Spread & Callback Allocation
**Learning:** Using array spread syntax paired with `Array.prototype.map()` purely to structure tuples for a `new Map()` constructor (e.g., `new Map([...document.querySelectorAll('.items')].map(item => [key, item]))`) causes double intermediate array allocations, resulting in excessive GC pressure and CPU cycles, particularly in functions called often or hot paths.
**Action:** Replace `Map` initialization using intermediary array tuple structures with a standard `new Map()` and a native `for` loop combined with `Map.prototype.set()` to populate the values, removing all allocation overhead. Also pair it with live HTMLCollections like `getElementsByClassName` when possible to avoid NodeList allocations.

...
## 2025-05-23 - [Complete Elimination of forEach and querySelectorAll loops in Hot Paths]
**Learning:** Even isolated instances of `Array.prototype.forEach` and `document.querySelectorAll(...).forEach` (e.g. in `initAutoSelectTextareas`, object key traversals `Object.keys().forEach`, event bindings `JC_CORPO_RECLASS_DOMAINS.forEach`) cumulatively trigger unnecessary callback allocations and C++ to JS boundary crossings.
**Action:** Always strictly use native standard `for` loops globally across the entire codebase for array iteration, NodeList iteration, and DOM queries to establish zero-allocation iterators by default, ensuring maximum performance stability.

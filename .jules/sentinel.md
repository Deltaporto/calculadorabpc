...
## 2025-05-05 - DoS and Slowloris Protection
**Vulnerability:** The local development server (`scripts/serve.js`) lacked URI length limits and explicit timeout configurations, leaving it vulnerable to URI DoS and Slowloris attacks.
**Learning:** Even simple local development servers (`http.createServer`) need explicit limits. Without `req.url.length` checks, long URIs can cause excessive parsing or memory issues. Without explicitly setting `server.setTimeout`, `server.keepAliveTimeout`, and `server.headersTimeout`, the server can be held open indefinitely by slow clients (Slowloris), potentially exhausting file descriptors.
**Prevention:** Always set reasonable global timeouts (`setTimeout`, `keepAliveTimeout`, `headersTimeout`) and validate incoming request limits (like URI length) early in the request handler lifecycle before any complex processing or parsing.

...
## 2025-09-08 - [MEDIUM] Missing input length limits on user input (DoS risk)
**Vulnerability:** The `<textarea>` inputs in the application (like `#jcAtivMedJustification`) lacked a `maxlength` attribute, making them susceptible to client-side DoS/memory exhaustion if extremely large strings (e.g., 10MB+) are pasted by the user.
**Learning:** Browser input components that lack size limits can lead to degraded performance or unresponsiveness when users supply excessively large inputs. Even without server-side validation, client-side input length limitations are important for application stability.
**Prevention:** Always add a reasonable `maxlength` attribute to `textarea` and text `input` elements to restrict the amount of text a user can enter and prevent excessive memory allocation during string processing.

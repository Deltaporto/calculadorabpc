...
export function calculateScore(state, domains, multiplier, pctToQFn) {
  // ⚡ Optimization: Native for-loop to avoid Array.prototype.reduce allocation overhead in hot paths
  let sum = 0;
...
export function computeAtivFromDomains(domains, ativDomainIds, pctToQFn = pctToQ) {
  ...
  // ⚡ Optimization: Native for-loop combines missing check and sum calculation, avoiding multiple array loops
  let sum = 0;

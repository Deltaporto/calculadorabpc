...
export function calculateScore(state, domains, multiplier, pctToQFn) {
  // ⚡ Optimization: Native for loop to avoid callback allocation overhead in hot path
  let sum = 0;
...
export function computeAtivFromDomains(domains, ativDomainIds, pctToQFn = pctToQ) {
  ...
  // ⚡ Optimization: Single native for loop with early return to avoid multiple iterations and callback allocations
  let sum = 0;

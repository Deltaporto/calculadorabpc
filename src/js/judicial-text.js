...
export function buildJudicialControlText({
  ...
  let reconhecimento;
  if (est && prog) {
    reconhecimento = 'Na avaliação administrativa, foram reconhecidas estruturas do corpo mais graves e prognóstico desfavorável em Funções do Corpo.';
  } else if (est && !prog) {
    reconhecimento = 'Na avaliação administrativa, foram reconhecidas estruturas do corpo mais graves, sem registro de prognóstico desfavorável em Funções do Corpo.';
  } else if (!est && prog) {
    reconhecimento = 'Na avaliação administrativa, não foram reconhecidas estruturas do corpo mais graves, porém houve registro de prognóstico desfavorável em Funções do Corpo.';
  } else {
    reconhecimento = 'Na avaliação administrativa, não foram reconhecidas estruturas do corpo mais graves nem prognóstico desfavorável em Funções do Corpo.';
  }
  ...
  if (triage.status === 'dispensa') {
    if (triage.route === 'sem_impedimento_lp' || m.impedimentoLP === false) {
      paragrafo2 = 'A perícia médica judicial não reconheceu impedimento de longo prazo, o que torna dispensável nova avaliação social para reversão do resultado.';
  ...
  const paragrafo3 = triage.status === 'dispensa'
    ? `${dispensaRef} a prova médica judicial é suficiente para definição do requisito biopsicossocial, dispensando a avaliação social judicial nesta fase processual.`
    : 'Nesse contexto, a avaliação social judicial é necessária para possibilitar a requalificação de Fatores Ambientais e Atividades e Participação e permitir cognição probatória completa sobre o requisito biopsicossocial.';

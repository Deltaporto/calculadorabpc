...
function ensureRefs() {
  ...
  // ⚡ Optimization: Pre-build static DOM nodes for dynamic math strings to prevent repeated fragment allocation on hot paths
  const initMathSpan = (container, parts) => {
    const nodes = [];
    if (container) {
  ...
  const fAmbNodes = initMathSpan(getById('fAmb'), [
    { type: 'text', content: '(' },
    { type: 'span', className: 'val' }, // sum
    { type: 'text', content: ' × 5) − 0,1 = ' },
    { type: 'span', className: 'val' }  // pct
  ]);
  const fCorpoNodes = initMathSpan(getById('fCorpo'), [
    { type: 'text', content: 'Máximo dos domínios: ' },
    { type: 'span', className: 'val' }, // max
    { type: 'text', content: '' },      // spacer
    { type: 'span', className: 'val' }, // final
    { type: 'text', content: '' }       // suffix
  ]);
  const fAtivNodes = initMathSpan(getById('fAtiv'), [
    { type: 'text', content: '(' },
    { type: 'span', className: 'val' }, // sum
    { type: 'text', content: ' × 2,778) − 0,1 = ' },
    { type: 'span', className: 'val' }  // pct
  ]);
  const refs = {
    fAmb: getById('fAmb'),
  ...
  // ⚡ Optimization: Use live HTMLCollection via getElementsByClassName and native for-loop to avoid querySelectorAll and Map allocation overhead
...
export function runMainUpdate({
  ...
  // ⚡ Optimization: Reuse existing cached DOM nodes to prevent layout thrashing and allocation
  ...
  setTextIfChanged(refs.fCorpoMax, String(corpo.max));
  setTextIfChanged(refs.fCorpoSpacer, corpo.majorado ? ' → ' : '');
